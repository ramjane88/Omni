    ...
    <div className="grid gap-4">
      <div className="card flex items-center justify-between">
        <div>
          ...
          <p className="text-white/70">Tap to mark done today. Simple local streaks.</p>
        </div>
      ...
      <div className="grid gap-3">
        {habits.map(h=>(
          <div key={h.id} className="card flex items-center justify-between">
            <div>
              ...
              <div className="text-xs text-white/60">Streak: {h.streak}</div>
            </div>
        ...
        {habits.length===0 && <div className="text-white/50">No habits yet. Add one.</div>}
      </div>
    </div>
  );
}
